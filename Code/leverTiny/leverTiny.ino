/**
    ...
    leverTiny.ino
    Purpose: Turns on a MOSFET when the input is triggered. Turn it off after five seconds to prevent overheating.
...
void loop() {
  if(millis() != time && outState == LOW) { // If more than one millisecond has elapsed since the last loop...
    ...
    if(counter >= DEBOUNCE_COUNT) { //  If the counter is greater than the debounce threshold...
      ...
      delay(5000); // Wait 5 seconds to ensure that the solenoid moves and the next transfer gets the signal
